...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'Basin', fields ['random_url_slug']
        db.create_unique('controlnext_basin', ['random_url_slug'])

        # Copying grower_info owner names to basin.
        for record in orm['controlnext.basin'].objects.all():
            owner = str(orm['controlnext.growerinfo'].objects.get(id=record.owner_id).name)
            record.owner_new = owner
            record.save()

        # Adding field 'WaterDemand.basin'
        db.add_column('controlnext_waterdemand', 'basin',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['controlnext.Basin'], null=True),
                      keep_default=False)
        # Adding names
        for record in orm['controlnext.waterdemand'].objects.all():
            basin = orm['controlnext.basin'].objects.get(id=record.owner_id)
            record.basin = basin
            record.save()


    def backwards(self, orm):
        # Removing unique constraint on 'Basin', fields ['random_url_slug']
        db.delete_unique('controlnext_basin', ['random_url_slug'])
    ...
    models = {
        ...
        'controlnext.basin': {
            ...
            'random_url_slug': ('django.db.models.fields.CharField', [], {'default': "u'45nkin0x54aqgeh6xv03'", 'unique': 'True', 'max_length': '20'}),
        ...
        'controlnext.growerinfo': {
            ...
            'rain_flood_surface': ('django.db.models.fields.IntegerField', [], {'null': 'True', 'blank': 'True'}),
            'rain_location_id': ('django.db.models.fields.CharField', [], {'max_length': '100', 'null': 'True', 'blank': 'True'}),
            'random_url_slug': ('django.db.models.fields.CharField', [], {'default': "u'zo1pstybkwxge3xz3rzi'", 'unique': 'True', 'max_length': '20'})
        },
        ...
        'controlnext.waterdemand': {
            ...
            'basin': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['controlnext.Basin']", 'null': 'True'}),
