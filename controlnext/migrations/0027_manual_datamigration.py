...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        for record in orm['controlnext.basin'].objects.all():
            old = record.owner_old
            record.owner = orm['controlnext.growerinfo'].objects.get(name=old)
            record.save()
    ...
    def backwards(self, orm):
        ...
        for record in orm['controlnext.basin'].objects.all():
            new_ = record.owner_id.id
            record.owner_old = orm['controlnext.growerinfo'].objects.get(
                id=new_).name
            record.save()
    ...
    models = {
        ...
        'controlnext.growerinfo': {
            ...
            'location': ('django.contrib.gis.db.models.fields.PointField', [], {'null': 'True', 'blank': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '100', 'null': 'True', 'blank': 'True'}),
            'random_url_slug': ('django.db.models.fields.CharField', [], {'default': "u'z6e8z87rpve633sdpzws'", 'unique': 'True', 'max_length': '20'})
        },
