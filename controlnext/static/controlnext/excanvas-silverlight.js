...
(function () {
  ...
  function processFontStyle(styleString) {
    ...
    if (style.fontFamily && (style.fontFamily.indexOf(" ") != -1 || style.fontFamily.indexOf('"') != -1)) {
        throw Error('No support for complex font families.');
    }
  ...
  contextPrototype.createPattern = function() {
    ...
    contextPrototype.measureText = function(txt) {
        var text;
        var s = '<TextBlock Text="' + txt + '"/>';
        try {
          text = create(this, s);
        } catch (ex) {
          throw Error('Could not create XAML from: ' + s);
        ...
        text.FontWeight = fontStyle.weight;
        text.FontSize = fontStyle.size;
        text.FontFamily = fontStyle.family;
        return {width: text.ActualWidth};
    };
  
    contextPrototype.drawText_ = function(txt, x, y, maxWidth, stroke) {
        var scaleWidth = 1;
        var text = this.measureText(txt);
  
        var fontStyle = getComputedStyle(processFontStyle(this.font),
                                       this.canvas);
        var elementStyle = this.canvas.currentStyle;
  
        if(maxWidth && (width < text.width)) {
          scaleWidth = width / text.width;
        }
        ...
        case 'right':
          ...
          break;
        case 'end':
            x = elementStyle.direction == 'ltr' ? x - text.width : x;
          break;
        case 'start':
        default:
            x = elementStyle.direction == 'rtl' ? x - text.width : x;
          break;
        }
        ...
        case 'ideographic':
        case 'bottom':
          y += -fontStyle.size / 2.25;
          break;
        }
  
        if(fontStyle.variant == 'small-caps') {
          txt = txt.toLowerCase();
        }
  
        var text = drawShape(this,
                '<TextBlock' +
                '  Text="%1"' +
                '  FontFamily="%5"' +
                '  FontSize="%6"' +
                '  FontWeight="%7"' +
                '  FontStyle="%8"' +
                '  Canvas.Left="%2" Canvas.Top="%3">' +
                '  <TextBlock.RenderTransform>' +
                '    <ScaleTransform ScaleX="%4" />' +
                '  </TextBlock.RenderTransform>' +
                '</TextBlock>',
                [txt, x, y, scaleWidth,
                 fontStyle.family, fontStyle.size,
                 fontStyle.weight, fontStyle.style]);
        if(!stroke) {
          text.Foreground = createBrushObject(this, this.fillStyle);
        }
    };
  
    contextPrototype.fillText = function(txt, x, y, maxWidth) {
        this.drawText_(txt, x, y, maxWidth, false);
    };
  
    contextPrototype.strokeText = function(txt, x, y, maxWidth) {
        this.drawText_(txt, x, y, maxWidth, true);
    };
